...
- Generate a companion ===COMPANION_FILE: <CopyBookName>.java=== with a Java @Entity class
  (if it maps to a DB/VSAM record) OR a plain DTO/POJO (if it is purely a data layout).
- Also generate a companion ===COMPANION_FILE: <CopyBookName>Repository.java=== JpaRepository
  interface if the copybook represents a persistent record.
