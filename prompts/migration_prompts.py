...
{copybooks_text}
Each copybook is a shared data layout (like a COBOL struct/record). For any copybook listed,
choose the companion type based on whether this paragraph does actual persistence I/O:

- If the DB2 TABLES / FILES section above is NON-EMPTY, or the COBOL source contains
  EXEC SQL, OPEN/READ/WRITE file verbs, or EXEC CICS READ/WRITE/REWRITE:
    → Generate a ===COMPANION_FILE: <CopyBookName>.java=== annotated with @Entity,
      @Table(name = "<copybook_name_snake_case>"), @Id, @GeneratedValue.
    → Include: import jakarta.persistence.*;
    → Generate a ===COMPANION_FILE: <CopyBookName>Repository.java=== JpaRepository interface.
    → @Autowired the repository in ===FIELDS===.

- If the DB2 TABLES / FILES section is "(none)" AND there is no EXEC SQL / file I/O
  in the COBOL source (the paragraph only moves/computes in-memory values):
    → Generate a ===COMPANION_FILE: <CopyBookName>.java=== as a plain DTO/POJO —
      private fields with public getters and setters only. No @Entity, no @Repository.
    → Do NOT add a Repository companion file.
    → Do NOT @Autowired anything for this copybook in ===FIELDS===.
    → Declare the DTO as a private field on the service class (instance state).
