...
{copybooks_text}
Each copybook is a shared data layout (like a COBOL struct). For any copybook listed:
- ALWAYS generate a companion ===COMPANION_FILE: <CopyBookName>.java=== annotated with
  @Entity, @Table(name = "<copybook_name_snake_case>"), @Id, and @GeneratedValue.
  Default to @Entity — do NOT generate a plain DTO/POJO unless the copybook name clearly
  indicates a non-persistent control/working-storage layout (e.g. WORK-AREA, WS-CTRL).
- Always include: import jakarta.persistence.*; in the companion file.
- Always generate a companion ===COMPANION_FILE: <CopyBookName>Repository.java===
  JpaRepository interface alongside every @Entity companion.
- Use the copybook name (CamelCase) as the class name.
